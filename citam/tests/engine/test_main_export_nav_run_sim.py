...
def test_export_navigation_graph_to_svg_no_issue(datadir, tmpdir, monkeypatch):
    ...
    main.export_navigation_graph_to_svg(
        facility="TEST", floor="0", outputfile=svg_file
    )
...
def test_run_simulation_no_issues(datadir, tmpdir, monkeypatch):
    inputs_dict = {
        ...
        "shifts": [{"name": "1", "start_time": 100, "percent_workforce": 1.0}],
    ...
    contact_dist_file = os.path.join(
        floor_subdir, "contact_dist_per_coord.csv"
    )
