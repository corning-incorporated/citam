...
def test_update_from_SVG_data(rect_floorplan, datadir):
    ...
    door_paths = [Line(start=complex(0, 20), end=complex(0, 60)),
                  Line(start=complex(125, 20), end=complex(125, 60))
                  ]
    wall_paths = [Line(start=complex(125, 0), end=complex(125, 80)),
    ...
    walls_removed = [w for w in orginal_fp.walls
                     if w not in fp_updater.floorplan.walls
                     ]
