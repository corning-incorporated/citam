...
def test_no_meetings(simple_facility_model, request, tmpdir):
    ...
    office_scheduler = OfficeScheduler(
        ...
        simple_facility_model.timestep,
        simple_facility_model.total_timesteps,
        simple_facility_model.scheduling_rules,
        None,
        simple_facility_model.buffer,
    )
    shift = {"name": "test", "percent_agents": 1, "start_time": 0}
    office_scheduler.run(3, shifts=[shift])
