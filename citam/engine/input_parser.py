...
def parse_input_file(input_file):
    ...
    if 'meetings_file' in input_dict:
        if os.path.isfile(input_dict['meetings_file']):
            meetings_policy = \
                parse_meetings_policy_file(input_dict['meetings_file'])
        else:
    ...
    if 'traffic_policy' in input_dict:
        if not isinstance(input_dict['traffic_policy'], list):
            raise TypeError('traffic_policy must be a list. Found {%s}',
                            type(traffic_policy))

        for pol in input_dict['traffic_policy']:
            if not isinstance(pol, dict):
                raise TypeError('A dictionary with these keys expected:' +
