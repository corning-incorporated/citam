...
class MeetingPolicy:
    ...
    def __init__(self,
        ...
        self.meeting_duration_increment = \
            policy_params['meeting_duration_increment']

        # Meetings frequency
        self.avg_meetings_per_room = policy_params['avg_meetings_per_room']
        self.avg_percent_meeting_rooms_used = \
            policy_params['avg_percent_meeting_rooms_used']

        # Meetings participants
        self.avg_meetings_per_person = policy_params['avg_meetings_per_person']
        self.min_attendees_per_meeting = \
    ...
    def create_meetings(self):
        ...
        tot_blocks = \
            int(self.max_meeting_length/self.meeting_duration_increment)
        ...
        for floor_rooms in self.meeting_rooms:
            ...
            for meeting_room in pb.progressbar(floor_rooms):
                ...
                for i in range(n_meetings):
                    ...
                    start_time = \
                        last_meeting_end_time + np.random.randint(3600)
                    ...
                    # Create new meeting
                    new_meeting = Meeting(location=meeting_room,
                    ...
                    capacity = 25
                    n_attendees = \
                        np.random.randint(self.min_attendees_per_meeting,
                                          capacity
                                          )
                    ...
                    if n_attendees >= len(pot_attendees):
                        ...
                        attendees = np.random.choice(pot_attendees,
                                                     n_attendees
                                                     )
    ...
    def _update_attendee_pool(self):
        ...
        while current_avg > self.avg_meetings_per_person:
            ...
            self.attendee_pool = {key: val for key, val in
                                  self.attendee_pool.items()
                                  if val < max_meetings
                                  }
