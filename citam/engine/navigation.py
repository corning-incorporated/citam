...
class Navigation:
    ...
    def get_multifloor_route(
        ...
        if self.multifloor_navnet.has_node(start_node):
            ...
            logging.error(
                "Start coordinates not in navnet: " + str(start_node)
            )
    ...
    def get_route_same_floor(
        ...
        if not self.navnet_per_floor[floor_number].has_node(start_coords):
            logging.error(
                "Start coordinates not in nav network: " + str(start_coords)
            )
            return None
        ...
        if not self.navnet_per_floor[floor_number].has_node(end_coords):
            ...
            logging.error(
                "End coordinates not in nav network:"
                + str(end_coords)
                + " Destination: "
                + dest_name
            )
